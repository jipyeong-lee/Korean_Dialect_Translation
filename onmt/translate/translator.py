...
class Translator(object):
    ...
    def translate(
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                if attn_debug:
                    ...
                    attns_np = attns_np[:, 0:len(srcs)]
                    
                    attn_df = pd.DataFrame(attns_np[::-1])
                    attn_df.columns = srcs
                    attn_df.index = reversed(preds)
                    
                    if hm_index != 3:
                        sns.heatmap(attn_df, ax=ax[hm_index], cbar=False)
                    else:
                        sns.heatmap(attn_df, ax=ax[hm_index])
                        
                    if self.logger:
                        self.logger.info(output)
                        #attn_hm.get_figure().savefig(f"data/heatmap/result{hm_index}.png")
                    else:
                        os.write(1, output.encode('utf-8'))
                        #attn_hm.get_figure().savefig(f"data/heatmap/result{hm_index}.png")
                    
                    #attn_hm.get_figure().clf()
                    hm_index += 1
        ...
        end_time = time.time()
        fig.savefig("data/heatmap/result.png")
